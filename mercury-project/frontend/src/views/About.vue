<template>
  <div class="about">
    <pieChartCard :suspects="this.pplOfIntrest.filter(person=> !person.is_wanted).length" :wanted="this.pplOfIntrest.filter(person=> person.is_wanted).length"/>
  </div>
</template>
...
export default {
  ...
  methods:{
    initPplArr(){
      //api
      this.pplOfIntrest =[{first_name: "ישראל", last_name:"ישראלי", id: 456346, 
